    ...
    for( i=0; i<VertexNum-1; i++ ) {  //最多顶点数个连通分量，外层循环提供了连通分量的编号，以递增的方式。
        for(j=0; j<EdgNum; j++ ) {  
            if( !(index[p[j].begin]>=0 && index[p[j].end]>=0 && index[p[j].begin]==index[p[j].end]) ) {  //找到当前还没加入到同一个连通分量且权值最小的边。p中的边已经从小到大排序。  
                ...
                if( (___________________) && (___________________) )          //该边的两个顶点都没加入过任何一个连通分量，为index值为-1 
                    ...
                    for(int n=0; n<VertexNum; n++ )  
                        if( index[n] == _________ )  //把尾所在的连通分量的所有顶点都设为i
                            index[n] = _________;  
                }  
  
                else if( (___________________) && (___________________) ) { //该边的头begin已在一个连通分量，尾end未加入过任何连通分量  
                    index[p[j].end] = i;  
