...
void DijkstraPath(Graph G, int *dist, int *pre, int v0)
{
	...
	for (i = 0; i < G.NumVertex; i++)  //初始化v0顶点到其它各顶点的距离
	{
		...
		{
			dist[i] = G.Edge[v0][i];
			pre[i] = v0;     //pre记录最短路径上从v0到i的前一个顶点
		}
		else
		{
			dist[i] = INFINITY;    //若i不与v0直接相邻，则权值置为无穷大
			pre[i] = -1;
		}
		visited[i] = FALSE;
		pre[v0] = v0;
	...
	for (i = 1; i < G.NumVertex; i++) //循环扩展n-1次
	{
		...
		for (k = 0; k < G.NumVertex; k++)   //更新dist数组的值和路径的值
		{
			...
			if (visited[k] == FALSE && G.Edge[u][k] > 0 && (dist[u] + G.Edge[u][k]) < dist[k])
			{
				dist[k] = dist[u] + G.Edge[u][k];//路径有更新就记录一下这一步加入访问集合的顶点
				pre[k] = u;
			}
		}
	}
