...
{  
    ...
    printf("请输入相应的的邻接矩阵，0表示无连接，按行输入:\n");  
...
{  
    ...
    for( i=0; i<VertexNum; i++ )   //边集数组初始化,由图的矩阵取值，同样只考虑上三角矩阵 
        for( j=i; j<VertexNum; j++ )  
            if( 0 != g.edg[i][j] ) {  //0表示无连接
                p[k].begin = i;  
                p[k].end = j;  
...
{  
    ...
    int *MSTEdge;      //MSTEdge数组，用来存储已确定的MST的边的编号，共VertexNum-1条边  
    int k= 0;  
    int WeightSum = 0;  
    int IndexBegin, IndexEnd;  
	
	VertexNum = CalVerNum(g);
	EdgNum = CalEdgNum(g);
	p = CreateEdges(g);
	index = (int *)malloc(sizeof(int)*VertexNum);
	MSTEdge = (int *)malloc(sizeof(int)*VertexNum - 1);

	for(i=0; i<VertexNum; i++ )      //初始化所有index = -1  
           index[i] = -1;  
  
    for( i=0; i<VertexNum-1; i++ ) {  //最多顶点数个连通分量，外层循环提供了连通分量的编号，以递增的方式。
        for(j=0; j<EdgNum; j++ ) {  
            if( !(index[p[j].begin]>=0 && index[p[j].end]>=0 && index[p[j].begin]==index[p[j].end]) ) {  //找到当前还没加入到同一个连通分量且权值最小的边。p中的边已经从小到大排序。  
                MSTEdge[i] = j;     //将其加入到MST中去  
  
                if((index[p[j].begin]==-1) && (index[p[j].end] ==-1))           //该边的两个顶点都没加入过任何一个连通分量，为index值为-1 
                    index[p[j].begin] = index[p[j].end] = i; //设起点与终点的编号为i     
  
                else if( (-1 == index[p[j].begin]) && (index[p[j].end] >= 0) ) { //该边的尾end已在一个连通分量，头begin未加入过任何连通分量  
                    index[p[j].begin] = i; //把头设为i 
                    IndexEnd = index[p[j].end];  
                    for(n=0; n<VertexNum; n++ )  
                        if( index[n] == IndexEnd )  //把尾所在的连通分量的所有顶点都设为i
                            index[n] = i;  
                }  
  
                else if( (index[p[j].end==-1]) && (index[p[j].begin]>=0) ) { //该边的头begin已在一个连通分量，尾end未加入过任何连通分量  
                    index[p[j].end] = i;  
                    IndexBegin = index[p[j].begin];  
                    for(n=0; n<VertexNum; n++ )  //把头所在的连通分量的所有顶点都设为i
