...
//                Thread.sleep(10);//下载过快时使用进程休眠便于观察下载进度
    ...
    // 下载文件
    public static File downloadFile(String path, String cachePath,CirclePercentView mCirclePercentView) throws Exception {
        // 判断缓存文件夹是否存在
        ...
        FileOutputStream fileOutputStream = null;//文件输出流
        if (is != null) {
            fileOutputStream = new FileOutputStream(cacheFile);//指定输出数据流的文件
            byte[] buf = new byte[1024];//缓存区
            int ch;//读取的字节数量
            long loadedLength = 0;//已下载文件数据大小
            while ((ch = is.read(buf)) != -1) {
                // 计算下载比例
                loadedLength = loadedLength + ch;
//                Thread.sleep(10);//下载过快时使用进程休眠便于观察下载进度
                fileOutputStream.write(buf, 0, ch);//将获取到的流写入文件中
                // 更新UI
                double loadedPercent = (loadedLength / (double) contentLength) * 100;
                mCirclePercentView.mLoadedPercent = loadedPercent;
            }
        }
        if (fileOutputStream != null) {
            fileOutputStream.flush();
