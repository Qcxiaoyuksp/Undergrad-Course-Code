...
void CreatGraph_DN(ALGraph *G)
{
	Acnode *p, *q; int i, j, sign1, sign2, Value; VertexNode v1, v2;//弧头弧尾
	char buf[INFINITY];  /*缓冲区*/
 	FILE *fp;            /*文件指针*/
 	int len;             /*行字符个数*/
 	...
 	//while(fgets(buf,INFINITY,fp) != NULL)
		/*buf"123\n";
 		len = strlen(buf);
 		buf[len-1] = '\0';  /*去掉换行符
 		printf("%s  \n",buf); */
 		...
 		printf("顶点数&边数：%d %d\n",G->Vexnum,G->Arcnum);
 		for (i = 0; i<G->Vexnum; i++)  {  
              fscanf(fp," %d",&G->Vertex[i].data);
			  printf("%d ",G->Vertex[i].data);
              G->Vertex[i].firstarc= NULL;  }  //头结点第一个邻接点赋空
 		for (i = 0; i<G->Arcnum; i++) { //插入每个弧
          sign1 = -1; sign2 = -1;
          fflush(stdin);
          printf("顶点数 边数 权值\n");
	 fscanf(fp,"%d%d%d",&sign1,&sign2,&Value);
	 printf("%d      %d   %d\n",sign1,sign2,Value);
         ...
         if (sign1 == -1 || sign2 == -1)   printf("信息输入有误！\n");
         else {   p = (Acnode*)malloc(sizeof(Acnode));  //申请空间
                     p->next = NULL;
                     p->adjvex = sign2;
                    ...
                    else{   while (q->next != NULL) //找到最后一个结点
	                q = q->next;   
                    q->next = p;}
        }
        }
...
void TopologicalSort1(ALGraph G)
{
	...
	for (i = 0; i<G.Vexnum; i++)//找到indegree数组中入度为0的顶点然后入栈
	       if (!indegree[i]){ e.adjvex = i;	push(s,e.adjvex); }
	while (!IsEmpty(s))
	{                  e.adjvex = pop(s);
		printf("%c ", G.Vertex[e.adjvex].data);
		count++;//对输出的顶点计数
		for (p = G.Vertex[e.adjvex].firstarc; p != NULL; p =p->next ) //用p遍历每个表结点
		{
			k = p->adjvex;
			indegree[k]--;
			if (!indegree[k]) { e.adjvex = k;	push(s,e.adjvex); }
...
int TopologicalSortPath(ALGraph G, Stack T)
{
	...
	while (!IsEmpty(s))
	{
		...
		for (p = G.Vertex[e.adjvex].firstarc ; p != NULL; p = p->next )
		{
			...
			if ((VE[e.adjvex] + p->value ) > VE[k]){ //计算顶点最早发生时间
				VE[k] = (VE[e.adjvex] + p->value );
			}
		}
	}
	return count;	
	...
	else
	{
		...
		while (!IsEmpty(T))
		{
			...
			if (G.Vertex[e.adjvex].firstarc != NULL)
			{
				...
				min = VL[p->adjvex] - p->value ;
				for (p = G.Vertex[e.adjvex].firstarc; p != NULL; p = p->next)
				{
					if (min > VL[p->adjvex] - p->value )
					min = VL[p->adjvex] - p->value ;
				}
