...
Status insertFront(cirDeque *Q, QElemType e)
{
	...
	if ((Q->front - 1) % MAXQSIZE == Q->rear)
		return ERROR;
	Q->front = (________________________) % MAXQSIZE;
	Q->base[_________________] = e;
...
QElemType getFront(cirDeque *Q){
	if (isEmpty(Q)){
		...
		return Q->base[__________________];
}
...
QElemType getRear(cirDeque *Q){
	if (isEmpty(Q)){
		...
		return Q->base[____________________________________];
}
...
Status EnMonoQueue(cirDeque *Q, QElemType e){
	while (__________________&& __________________){
        deleteRear(Q);
	}
	insertRear(Q, e);
}
...
int * maxSlidingWindow(cirDeque *Q, int *nums, int numsSize, int k){
	...
	for (i = k; i < numsSize; i++) {
		// 窗口开始向前滑动
		DeMonoQueue(Q, __________________); // 窗口后端
		EnMonoQueue(Q, __________________);// 窗口前端
		result[__________________] = maxMonoQueue(Q); // 记录对应的最大值，第几个结果
	}
	return result;
}
