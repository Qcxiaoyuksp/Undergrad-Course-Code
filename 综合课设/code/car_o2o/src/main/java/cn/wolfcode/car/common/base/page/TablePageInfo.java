...
public class TablePageInfo<T>{
    public static TablePageInfo EMPTY_DATA = new TablePageInfo(0L, Collections.EMPTY_LIST);
    private int code;           //返回值状态码：为0是表示成功
    private String msg;         //返回值信息
    private long total;         //分页总条数
    private List<T> rows;       //当前页面显示数据
    ...
    public TablePageInfo(Long total, List<T> list){
        this.total = total;
        this.rows = list;
        this.code = StateType.SUCCESS.getValue();
    }
