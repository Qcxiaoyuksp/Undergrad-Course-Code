...
void Floyd(Graph g, int **path,int **length)
{
	...
	for (i = 0; i < n; i++){ //初始化两两顶点的距离
       for (j = 0; j<n; j++)
	   {
		   length[i][j] = g.____________;
		   if (length[i][j] < ____________){ //两顶点的相连
			   path[i][j]=____________;
		   }
		   else
		       path[i][j]= -1;  //两顶点的不相连
	   }
	...
	for (k = 0; k<n; k++)  //分别加入每个顶点k检查最短径变化
	{
	   for (i = 0; i<n; i++)
		  for (j = 0; j<n; j++)
		    if (length[i][j]>(____________ + ____________))
		    {
		       length[i][j] = ____________ + ____________;
			   path[i][j] = ____________;
		    }
	}
}
