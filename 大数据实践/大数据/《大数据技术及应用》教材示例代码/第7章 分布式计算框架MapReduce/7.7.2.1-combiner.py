...
def main(separator='\t'):
    ...
    for words in wd:
        ...
        if(city == current_city):
            ...
            if counter>0:  #城市切换，且非第一条数据，则需输出上步骤结果
                print("{0}{4}{1}{4}{2}{4}{3}{4}{5}".format(current_city, maxv, minv, sumv, separator, counter))  #修改输出为sumv
            maxv, minv, sumv, counter = float(words[1]), float(words[2]), float(words[3]), int(words[4])
            current_city = words[0]
    output ="{0}{4}{1}{4}{2}{4}{3}{4}{5}".format(current_city, maxv, minv, sumv, separator, counter)  #修改输出为sumv
