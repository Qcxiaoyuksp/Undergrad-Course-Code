...
void CreatGraph_DN(ALGraph *G)
{
    ...
    for(i = 0; i < G->Vexnum; ++i)
    {
        fflush(stdin); //清空输入缓冲区，为了不影响后面的数据读取 
        scanf("%c ",&G->Vertex[i].data); //输入顶点的信息 
    ...
    for(k = 0; k < G->Arcnum; ++k)
    {
        int i,j,w,ii,jj;
        char vi,vj;
        printf("请输入边的连接顶点信息(vi,vj)和边的权值：\n");
        fflush(stdin);
        scanf("%c %c %d",&vi,&vj,&w);
        for(i=0; i<G->Vexnum; i++)
        {
            if(G->Vertex[i].data==vi)
                ii=i;
        }
        for(j=0; j<G->Vexnum; j++)
        {
            if(G->Vertex[j].data==vj)
                jj=j;
        }
        Acnode *p=(Acnode *)malloc(sizeof(Acnode));
        p->adjvex=jj;
        p->value=w;
        p->next=G->Vertex[ii].firstarc;
        G->Vertex[ii].firstarc=p;
    }
}
