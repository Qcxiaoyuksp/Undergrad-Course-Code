...
public:
	...
	void modify();      //定义修改函数；
	void del();         //定义删除函数;
	void statistics_class();  //定义一个统计函数；统计班级人数；
	void statistics_major();  //定义一个统计函数；统计专业人数；
...
void entrance()
{
	...
	switch(a)
	{
		...
		case 5:             //统计班级人数 
			cout<<"\t\t\t欢迎进入统计班级人数界面\n\n";
			stu.statistics_class();
			break;
		case 6:             //统计专业人数 
			cout<<"\t\t\t欢迎进入统计专业人数界面\n\n";
			stu.statistics_major();
			break;
		case 7:
			stu.showAll();
			break;
...
void Student :: statistics_major()
{
	...
	for (int i = 0; i < sum; i++)
	{
		if (s == vec[i].major) count++;
	}
	
	cout << s << "专业的学生个数是：" << count << endl;
